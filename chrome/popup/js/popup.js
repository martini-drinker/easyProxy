...
function buildContent(pick) {
	...
	if (pick === `controls`) {
		...
		let isAuthDisabled = settings.type === `socks4` || settings.type === `socks5`;

		ids[`settings-auth`].classList.toggle(`disabled`, isAuthDisabled);
		ids[`settings-auth`].querySelector(`input`).checked = settings.auth;

		let isAuthFieldsDisabled = isAuthDisabled || !settings.auth;

		ids[`settings-username`].classList.toggle(`disabled`, isAuthFieldsDisabled);
		ids[`settings-username`].querySelector(`input`).value = settings.username;
		ids[`settings-password`].classList.toggle(`disabled`, isAuthFieldsDisabled);
...
function addListeners() {
	...
	ids[`settings-type`].querySelector(`select`).addEventListener(`change`, e => {
		...
		if (settings.type === `socks4` || settings.type === `socks5`) {
			settings.auth = false;
		}
